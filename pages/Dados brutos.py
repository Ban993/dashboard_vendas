...
def mensagem_sucesso():
    sucesso = st.success('Aquivo baixado com sucesso', incon="✅")
    time.sleep(5)
    sucesso.empty()
...
with st.expander('Colunas'):
    colunas = st.multiselect('Selecione a coluna', list(dados.columns), list(dados.columns))
...
with coluna2:
    st.download_button('Fazer o downkiad da tabela em csv', data=convert_csv(dados_filtrados), file_name= nome_arquivo, mime= 'text/csv', on_click= mensagem_sucesso)
